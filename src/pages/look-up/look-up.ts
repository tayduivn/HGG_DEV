...
export class LookUpPage extends PageBase {
    ...
    isLocationAvailable()
    {
        ...
        this.showLoading();
        var loc = this.geolocation.getCurrentPosition(geolocationOptions).then(loc => {
        ...
        // if(this.platform.is('ios')) {
        //     this._DIAGNOSTIC.isLocationEnabled().then(async (isEnabled) => {
        //         if(!isEnabled) {
        ...
        //         } else {
        //             this.showLoading();
        //             var loc = await this.geolocation.getCurrentPosition(geolocationOptions);
        //             // //Your logic here
        //             this.latitude  = loc.coords.latitude;
        //             this.longitude = loc.coords.longitude;
        ...
        //     let alert = this.alertCtrl.create({
        //         title: this.translateService.translate("error.title"),
        //         subTitle: this.translateService.translate("gps.error") + " " + error.message,
        //         buttons: [{
        //             text: "OK",
        //             handler: () => {
        //                 this._DIAGNOSTIC.switchToLocationSettings();
        ...
        //                 this.getResultType(this.latitude, this.longitude);
        //                 this._DIAGNOSTIC.registerLocationStateChangeHandler((locationMode) => {
        //                     if((this.platform.is("android") && locationMode !== this._DIAGNOSTIC.locationMode.LOCATION_OFF)
        //                         || (this.platform.is("ios") && ( locationMode === this._DIAGNOSTIC.permissionStatus.GRANTED
        //                             || locationMode === this._DIAGNOSTIC.permissionStatus.GRANTED_WHEN_IN_USE
        //                     ))) {
        //                         this.showLoading();
                                
        //                         this.geolocation.getCurrentPosition({timeout: 20000, maximumAge: 10000, enableHighAccuracy: true})
        //                         .then((data : any) =>
        //                         {
        //                             this.latitude  = data.coords.latitude;
        //                             this.longitude = data.coords.longitude;
    ...
    updateType(ts) {
        ...
        // .then((isAvailable) => {
        //     this.isLocationEnabled = isAvailable;
    ...
    getResultType(latitude, longitude, loading = true) {
        ...
        var sta = localStorage.getItem("seen");
        var arr = this.stringtoArray(sta);
        this.searchLocationService.SearchByType(code, latitude + "," + longitude, 10, this.page_index, this.hotel_type).subscribe(
            data => {
                if (data.Code === 200) {
                    this.listResult = [];
    ...
    // loadHobbies() {
    //     this.hobbyService.GetList().subscribe(
    //         data => {
    //             if (data.Code === 200) {
    //                 this.listHobies = _.concat(this.listHobies, data.Result);
    //                 this.listHobies.forEach(element => {
    //                     this.listHobiesChoice.push(
    //                         {
    ...
    async loadServices() {
        ...
        if (this.searchType == "2") {
            ...
            this.serviceGetting.GetListFB().subscribe( // nha hang
                data => {
                    if (data.Code === 200) {
            ...
            // this.serviceGetting.GetListFBBestFor().subscribe( // nha hang
            //     data => {
            //         if (data.Code === 200) {
            //             //console.log(data.Result);
            //             this.listServicesBF = _.concat(this.listServicesBF, data.Result);
            //             this.listServicesBF.forEach(element => {
            //                 this.listServicesChoiceBestResFor.push(
            //                     {
            //                         name: element.NAME,
            //                         checked: false,
            //                         id: element.ID,
        ...
        if (this.searchType == "3") {
            this.serviceGetting.GetListTravel().subscribe(//diem tham quan
                data => {
                    if (data.Code === 200) {
                        this.listServices = _.concat(this.listServices, data.Result);
                        this.listServices.forEach(element => {
    ...
    updateHobbyChoice() {
        ...
        this.getResultFilter();
        // document.getElementById("hobby").setAttribute("style","visibility: hidden; opacity: 0; transform: translateY(-2em); z-index: 1; transition: all 0.3s ease-in-out 0s, visibility 0s linear 0.3s, z-index 0s linear 0.01s;");
        this.closeAllListMenu();
    }
    ...
    updateServiceChoice() {
        ...
        // this.listServicesChoiceBestResFor.forEach(element => {
        //     if(element.checked) {
        //         this.listServicesChoiceId.push(element.code + "_" + element.id);
        //     }
        // });
        this.getResultFilter();
        // document.getElementById("service").setAttribute("style","visibility: hidden; opacity: 0; transform: translateY(-2em); z-index: 1; transition: all 0.3s ease-in-out 0s, visibility 0s linear 0.3s, z-index 0s linear 0.01s;");
        this.closeAllListMenu();
    }
    ...
    loadListCategories() {
        ...
        if(this.searchType == "8") {
            this.listCategory = [];
            this.listCategoryChoice = [];
            this.hotelService.getEntertainmentType().subscribe(data => {
                if(data.Code === 200) {
                    data.Result.forEach(element => {
                        this.listCategoryChoice.push(
                            {
    ...
    updatingFunction(item) {
        ...
        if (item == null) {
            ...
            // if(this.restaurant_type != null) {
            //     if(this.listServicesChoiceId.length >= 0) {
            //         this.listServicesChoiceId.forEach((element, index) => {
            //             if(element == this.restaurant_type) {
            //                // console.log(this.restaurant_type);
            //                  this.listServicesChoiceId.splice(index, 1);
            //             }
            //         });
            //     }
            // }
            // if(this.sightseeing_type != null) {
            //     if(this.listServicesChoiceId.length >= 0) {
            //         this.listServicesChoiceId.forEach((element, index) => {
            //             if(element == this.sightseeing_type) {
            //                 // console.log(this.sightseeing_type);
            //                 this.listServicesChoiceId.splice(index, 1);
            //             }
            //         });
            //     }
            ...
            // if(this.searchType == "2") {
            //     this.hotel_type = null;
            //     this.listServicesChoiceId.push(item.code + "_" + item.id);
            //     // console.log(this.listServicesChoiceId);
            //     this.getResultFilter();
            // }
            // if(this.searchType == "1") {
            ...
            // }
            // if(this.searchType == "5") {
            if (this.searchType == "1" || this.searchType == "2" || this.searchType == "3" || this.searchType == "5" || this.searchType == 1 || this.searchType == 2 || this.searchType == 3 || this.searchType == 5) {
                this.hotel_type = item.id;
                this.getResultFilter();
            }
