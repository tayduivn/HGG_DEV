...
*/
@Component({
    selector: 'camera-emotion-page',
    templateUrl: 'camera-emotion.html'
})
export class CameraEmotionPage extends PageBase {
    ...
    private postData(formData: FormData) {
        ...
        this.http.post(Services.ServerURL("ChatBot/Emotion"), formData, { headers: headers })
            .map(response => Services.extractResult(response))
            .subscribe((data) => {
                ...
                if (data.Code == 200) {
                    ...
                    for (var i = 0; i < this.data.length; i++) {
                        ps += "<p>" + this._translate.translate(this.data[i].Name) + ": " + this.data[i].Value + "% </p>";
                    }

                    let confirm = this.alertCtrl.create({
                        title: this.translateService.translate('info.emotion'),
                        message: ps
                    });
                    confirm.present();
                }
